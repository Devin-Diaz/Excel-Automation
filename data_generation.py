...
def readExcelFile(file_path):
    ...
    data_frames = {sheet: pd.read_excel(file_path, sheet_name=sheet) for sheet in sheet_names}
    return data_frames


def createLineGraph(dataframe, x_col, y_col):
    plt.figure(figsize = (10, 6))
    sns.lineplot(data = dataframe, x= x_col, y = y_col)
    plt.title(f'Line Graph of {y_col} vs {x_col}')
    plt.xlabel(x_col)
    plt.ylabel(y_col)
    plt.show()
...
data_frames = readExcelFile(FILE_PATH)

for sheet_name, df in data_frames.items():
    if 'NumericalColumn1' in df.columns and 'NumericalColumn2' in df.columns:
        createLineGraph(df, 'NumericalColumn1', 'NumericalColumn2')
        break  # Break after the first test to avoid redundant plots
